...
router.get('/drivers', authMiddleware, async (_req, res) => {
  try {
    ...
    try {
      const fsList = await firestore.listDriverVerifications();
      firestoreOnly = fsList.filter((d) => d.driverId && !pgDriverIdSet.has(d.driverId));
      // Best-effort: backfill missing drivers into PostgreSQL so they persist and can be managed.
      for (const fsDriver of firestoreOnly) {
        try {
          const [pgRow, created] = await DriverVerification.findOrCreate({
            where: { driverId: fsDriver.driverId },
            defaults: {
              driverId: fsDriver.driverId,
              status: fsDriver.status || 'pending',
          ...
          if (created) {
            list.push(pgRow);
            pgDriverIdSet.add(fsDriver.driverId);
            console.info('[admin] backfilled Firestore driver to PG:', fsDriver.driverId);
          }
        } catch (backfillErr) {
          console.warn('[admin] backfill driver skipped:', fsDriver.driverId, backfillErr.message);
          // Still include in results even if PG backfill fails
          list.push({
            driverId: fsDriver.driverId,
            status: fsDriver.status || 'pending',
            vehicleType: fsDriver.vehicleType || 'car',
...
router.get('/drivers/:id', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (!row) {
      ...
      return res.json({
        driver: {
          ...
          hasAntecedentesPoliciales: fsRow.hasAntecedentesPoliciales ?? null,
          hasAntecedentesPenales: fsRow.hasAntecedentesPenales ?? null,
    ...
    // Lookup phone from AppUser by email
    let driverPhone = null;
    const driverEmail = row.email || null;
    if (driverEmail) {
      try {
        const appUser = await AppUser.findOne({ where: { email: driverEmail }, attributes: ['phone'], raw: true });
        if (appUser?.phone) driverPhone = appUser.phone;
      } catch (_) {}
    }
    return res.json({
      driver: {
...
/** PATCH /api/admin/drivers/:id/antecedentes – Update antecedentes yes/no fields. */
router.patch('/drivers/:id/antecedentes', authMiddleware, async (req, res) => {
  try {
    const driverId = req.params.id;
    if (!driverId) return res.status(400).json({ error: 'driverId required' });

    const body = req.body || {};
    const hasPol = body.hasAntecedentesPoliciales;
    const hasPen = body.hasAntecedentesPenales;

    if (hasPol == null && hasPen == null) {
    ...
    console.error('[admin] antecedentes update', err.message);
    return res.status(500).json({ error: err.message });
  }
});
...
/** GET /api/admin/drivers/:id/documents – List uploaded documents for a driver (admin access). */
router.get('/drivers/:id/documents', authMiddleware, async (req, res) => {
  try {
    const driverId = req.params.id;
    ...
    console.error('[admin] driver documents list', err.message);
