...
const { sendVerificationOtp, sendPasswordResetOtp, isConfigured: isMsg91Configured } = require('../services/msg91');
...
router.post('/signup', async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, message: isDev ? err.message : 'Signup failed' });
  }
});
...
router.post('/email-login', async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, message: isDev ? err.message : 'Login failed' });
  }
});
...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    if (isDev) console.log(`[auth] DEV: password-reset OTP for ${e} = ${otp}`);
